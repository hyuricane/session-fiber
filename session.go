...
func (s *Session) setSession() {
	sid := s.id
	if s.config.SignKey != nil {
		sid = s.config.SignKey(s.id)
	}
	if s.config.source == SourceHeader {
		s.ctx.Request().Header.SetBytesV(s.config.sessionName, []byte(sid))
		s.ctx.Response().Header.SetBytesV(s.config.sessionName, []byte(sid))
	} else {
		fcookie := fasthttp.AcquireCookie()
		fcookie.SetKey(s.config.sessionName)
		fcookie.SetValue(sid)
